...
async function scrapeGameBoxscore(gameId) {
  ...
  // Pick the Moeller table: prefer one with Moeller context, otherwise take first (Moeller is listed first)
  const moellerBattingTable = battingTables.find(t => t.isMoellerContext) || battingTables[0];

  // For pitching: the Moeller pitching table immediately follows the Moeller batting table.
  // Find the pitching table closest AFTER the Moeller batting table in DOM order.
  let moellerPitchingTable = pitchingTables.find(t => t.isMoellerContext);
  if (!moellerPitchingTable && moellerBattingTable) {
    const batIdx = moellerBattingTable.i;
    // Take the first pitching table that appears after the Moeller batting table
    moellerPitchingTable = pitchingTables.find(t => t.i > batIdx);
  }
