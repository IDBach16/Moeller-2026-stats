...
async function scrapeGameBoxscore(gameId) {
  ...
  //
  // The boxscore page renders each team's section as: Hitting, Pitching, Fielding.
  // The away team's section comes first, then the home team's section. So when Moeller
  // is HOME, the opponent's pitching table appears BEFORE Moeller's pitching table —
  // we cannot just take the first pitching table.
  //
  // Strategy: the Hitting (batting) table has the team name as its first <th>
  // (e.g., "Moeller"/"Milford"). Use that to find Moeller's batting table by DOM
  // index, then take the next pitching table that appears AFTER it.

  const batting = [];
  const pitching = [];

  // Collect all table info with their DOM index and team-name signals
  const tableInfos = [];
  allTables.forEach(($table, idx) => {
    const allHeaders = $table.find('th').map((i, th) => clean($(th).text())).get();
    const headerStr = allHeaders.join(' ');
    const firstHeader = (allHeaders[0] || '').toLowerCase();
    const hasMoellerTh = allHeaders.some(h => h.toLowerCase().includes('moeller'));
    // Only trust the immediately preceding sibling — checking the whole parent
    // matches every table on the page (entire body contains "Moeller").
    const prevText = clean($table.prev().text()).toLowerCase();
    ...
    tableInfos.push({
      ...
      headerStr,
      allHeaders,
      firstHeaderIsMoeller: firstHeader === 'moeller',
      hasMoellerTh,
      prevTextHasMoeller: prevText.includes('moeller')
    });
  });

  // Batting tables have AB and SLG columns
  const battingTables = tableInfos.filter(t => t.headerStr.includes('AB') && t.headerStr.includes('SLG'));
  // Pitching tables have IP and ER columns but no AB
  const pitchingTables = tableInfos.filter(t => t.headerStr.includes('IP') && t.headerStr.includes('ER') && !t.headerStr.includes('AB'));

  // Find Moeller batting table: prefer first <th> = "Moeller", then any th containing "Moeller"
  let moellerBattingTable = battingTables.find(t => t.firstHeaderIsMoeller)
    || battingTables.find(t => t.hasMoellerTh);

  // If only one batting table exists (non-GCL opponent — only Moeller stats shown), use it
  if (!moellerBattingTable && battingTables.length === 1) {
    moellerBattingTable = battingTables[0];
  }

  // Moeller's pitching table is the first pitching table that comes AFTER Moeller's batting table.
  let moellerPitchingTable = null;
  if (moellerBattingTable) {
    moellerPitchingTable = pitchingTables.find(t => t.idx > moellerBattingTable.idx);
  }
  // Fallback for single-team pages or if anchor failed: pick by direct preceding-sibling signal
  if (!moellerPitchingTable) {
    moellerPitchingTable = pitchingTables.find(t => t.prevTextHasMoeller);
  }
  if (!moellerPitchingTable && pitchingTables.length === 1) {
    moellerPitchingTable = pitchingTables[0];
  }
