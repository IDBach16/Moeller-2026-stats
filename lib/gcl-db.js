...
// ============================================================
//  GCL RIVALS — stats compiled against Elder / La Salle / St.X
// ============================================================
const RIVALS = ['Elder', 'La Salle', 'St. Xavier'];

function rivalList(rival) {
  if (rival && RIVALS.includes(rival)) return [rival];
  return RIVALS;
}

function getRivalsGames(rival = null) {
  const rivals = rivalList(rival);
  const placeholders = rivals.map(() => '?').join(',');
  return getDb()
    .prepare(`SELECT * FROM gcl_games WHERE opponent IN (${placeholders}) ORDER BY date ASC`)
    .all(...rivals);
}

function _rivalsBattingAggregate(rival) {
  const rivals = rivalList(rival);
  const placeholders = rivals.map(() => '?').join(',');
  return getDb()
    .prepare(`
      SELECT
        gb.player,
        MAX(gb.class_year) as class_year,
        COUNT(DISTINCT gb.gcl_game_id) as g,
        SUM(gb.ab) as ab,
        SUM(gb.r) as runs,
        SUM(gb.h) as hits,
        SUM(gb.rbi) as rbi,
        ...
        SUM(gb.hbp) as hbp,
        SUM(gb.sb) as sb
      FROM gcl_game_batting gb
      JOIN gcl_games gg ON gg.gcl_game_id = gb.gcl_game_id
      WHERE gg.opponent IN (${placeholders})
      GROUP BY gb.player
      HAVING SUM(gb.ab) + SUM(gb.bb) + SUM(gb.hbp) + SUM(gb.sf) + SUM(gb.sh) > 0
      ORDER BY SUM(gb.ab) DESC, SUM(gb.h) DESC
    `)
    .all(...rivals);
}

function getRivalsBatting(rival = null) {
  return _rivalsBattingAggregate(rival).map(row =>
    computeAdvancedBatting({ ...row, h: row.hits, cs: 0 })
  );
}

function getRivalsBattingTotals(rival = null) {
  const rows = _rivalsBattingAggregate(rival);
  if (rows.length === 0) return null;
  const sums = rows.reduce((acc, r) => {
    for (const k of ['g','ab','runs','hits','rbi','doubles','triples','hr',
                     'bb','so','sf','sh','hbp','sb']) {
      acc[k] = (acc[k] || 0) + (r[k] || 0);
    }
    return acc;
  }, {});
  // G should be distinct games, not the sum across players. Recompute.
  sums.g = getRivalsGames(rival).length;
  sums.player = 'TOTAL';
  sums.class_year = '';
  return computeAdvancedBatting({ ...sums, h: sums.hits, cs: 0 });
}

function _rivalsPitchingAggregate(rival) {
  const rivals = rivalList(rival);
  const placeholders = rivals.map(() => '?').join(',');
  return getDb()
    .prepare(`
      SELECT
        gp.player,
        MAX(gp.class_year) as class_year,
        COUNT(DISTINCT gp.gcl_game_id) as g,
        SUM(gp.ip_full) as ip_full_sum,
        SUM(gp.ip_partial) as ip_partial_sum,
        SUM(gp.h) as h,
        SUM(gp.r) as r,
        SUM(gp.er) as er,
        SUM(gp.bb) as bb,
        SUM(gp.so) as so,
        SUM(gp.hr) as hr
      FROM gcl_game_pitching gp
      JOIN gcl_games gg ON gg.gcl_game_id = gp.gcl_game_id
      WHERE gg.opponent IN (${placeholders})
      GROUP BY gp.player
      HAVING (SUM(gp.ip_full) * 3 + SUM(gp.ip_partial)) > 0
      ORDER BY ((SUM(gp.er) * 21.0) /
        NULLIF(SUM(gp.ip_full) * 3 + SUM(gp.ip_partial), 0)) ASC
    `)
    .all(...rivals);
}

function _pitchingFromAgg(agg) {
  const outs = (agg.ip_full_sum || 0) * 3 + (agg.ip_partial_sum || 0);
  const ip_full = Math.floor(outs / 3);
  const ip_partial = outs % 3;
  const ip_value = ip_full + ip_partial / 3;
  const era = ip_value > 0
    ? parseFloat(((agg.er * 7) / ip_value).toFixed(2)) : 0;
  const whip = ip_value > 0
    ? parseFloat(((agg.h + agg.bb) / ip_value).toFixed(2)) : 0;
  return computeAdvancedPitching(
    {
      player: agg.player,
      class_year: agg.class_year,
      g: agg.g,
      ip: `${ip_full}.${ip_partial}`,
      ip_full,
      ip_partial,
      w: 0, l: 0, sv: 0, sho: 0,
      k: agg.so,
    ...
    {
      h: agg.h, bb: agg.bb, so: agg.so, hr: agg.hr,
      er: agg.er, r: agg.r, hbp: 0,
    }
  );
}

function getRivalsPitching(rival = null) {
  return _rivalsPitchingAggregate(rival).map(_pitchingFromAgg);
}

function getRivalsPitchingTotals(rival = null) {
  const rows = _rivalsPitchingAggregate(rival);
  if (rows.length === 0) return null;
  const sums = rows.reduce((acc, r) => {
    for (const k of ['ip_full_sum','ip_partial_sum','h','r','er','bb','so','hr']) {
      acc[k] = (acc[k] || 0) + (r[k] || 0);
    }
    return acc;
  }, {});
...
module.exports = {
  ...
  getRivalsGames,
  getRivalsBatting,
  getRivalsBattingTotals,
  getRivalsPitching,
  getRivalsPitchingTotals,
};
